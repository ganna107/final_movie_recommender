...
st.markdown("""
    ...
    .movie-card { 
        border-radius: 10px; background-color: #1a1c23; padding: 10px; text-align: center; height: 100%;
        transition: transform 0.3s ease; border: 1px solid transparent;
    }
    .movie-card:hover { transform: scale(1.08); border: 1px solid #B20710; box-shadow: 0px 10px 20px rgba(178, 7, 16, 0.4); }
...
def astar_search(start_idx, target_idx):
    ...
    while queue:
        ...
        if vertex == target_idx: 
            return path, int(round(g * 100)), (time.time() - start_time) * 1000
...
def ucs_search(start_idx, target_idx):
    ...
    while queue:
        ...
        if vertex == target_idx: 
            return path, int(round(cost * 100)), (time.time() - start_time) * 1000
...
if st.button('Show Recommendations'):
    ...
    for i, m in enumerate(distances):
        with cols[i]:
            ...
            st.markdown(f'''<div class="movie-card"><img src="{fetch_poster(movie_id)}" style="width:100%; border-radius:5px;"><p style="color:white; font-size:0.8rem; margin-top:10px; font-weight:bold;">{df.iloc[m[0]].title}</p></div>''', unsafe_allow_html=True)

    if show_analysis:
        target_idx = df[df['title'] == target_movie].index[0]
        path, cost, exec_time = None, 0, 0
        
        if algo_type == "A*": path, cost, exec_time = astar_search(idx, target_idx)
        elif algo_type == "UCS": path, cost, exec_time = ucs_search(idx, target_idx)
        else: 
            path, exec_time = dfs_search(idx, target_idx)
            if path:
                cost = int(round(sum([1 - similarity[path[i]][path[i+1]] for i in range(len(path)-1)]) * 100))
        
        if path:
            st.sidebar.success(f"Algorithm: {algo_type}")
            st.sidebar.metric("Execution Time", f"{exec_time:.2f} ms") 
            st.sidebar.metric("Total Cost", f"{cost}") 
            ...
            ax_bar.set_ylabel("Score"); ax_bar.set_xlabel("Step")
            st.sidebar.pyplot(fig_bar)

            st.sidebar.write("### 📈 Path Similarity Flow")
            fig_line, ax_line = plt.subplots()
            ax_line.plot(path_sims, marker='o', color='#B20710', linewidth=2)
            ax_line.set_ylabel("Score"); ax_line.set_xlabel("Step")
